...
@export var primary_replacement_object : PackedScene = null
@export var secondary_replacement_object : PackedScene = null
...
func action_ongoing(delta: float) -> void:
	...
	if primary_hand.global_position.distance_to(secondary_hand.global_position) > requirement:
		...
		secondary_hand.get_node("FunctionPickup")._pick_up_object(new_primary_obj)
